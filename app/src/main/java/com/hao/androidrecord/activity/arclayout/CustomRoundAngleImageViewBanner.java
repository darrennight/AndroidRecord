 ...
 *      支持自定义四个角角度
 */
public class CustomRoundAngleImageViewBanner extends AppCompatImageView {
    private float width, height;
//    private int defaultRadius = 0;
    private int defaultRadius = 0;
    private int radius;
    private int radiusTOP = 0;
    private int radiusBottom = 50;
    private int leftTopRadius;
    private int rightTopRadius;
    private int rightBottomRadius;
    private int leftBottomRadius;

    public CustomRoundAngleImageViewBanner(Context context) {
        this(context, null);
        init(context, null);
    }

//    public CustomRoundAngleImageView(Context context, boolean[] radii) {
//        init(context, null);
//    }

    public CustomRoundAngleImageViewBanner(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        init(context, attrs);
    }

    public CustomRoundAngleImageViewBanner(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (defaultRadius == leftTopRadius) {
            leftTopRadius = radiusTOP;
        }
        if (defaultRadius == rightTopRadius) {
            rightTopRadius = radiusTOP;
        }
        if (defaultRadius == rightBottomRadius) {
            rightBottomRadius = radiusBottom;
        }
        if (defaultRadius == leftBottomRadius) {
            leftBottomRadius = radiusBottom;
        }
        array.recycle();
    }
    @Override
