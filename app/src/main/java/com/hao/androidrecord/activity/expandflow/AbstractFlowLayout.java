            ...
            if (child.getVisibility() != View.GONE) {
                ...
                if (xpos + childw > width) {
                    ...
                    if (mCurrentState == STATE_COLLAPSED &&
                        ...
                        if (xpos + childw + dotsViewWidth +HORIZONTAL_SPACING > width){
                            ypos -= mHeight + VERTICAL_SPACING;
                        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                if (xpos + childw + dotsViewWidth +HORIZONTAL_SPACING >= width &&
                        currentRow == ALLOWED_ROWS_COUNT && mCurrentState == STATE_COLLAPSED){
                    if (child.getId() != mDotsViewId){
                        child.setVisibility(View.INVISIBLE);
                        WeakReference<View> viewRef = new WeakReference<View>(child);
                        mLastRowViews.add(viewRef);
                    }
                    //当前这个item child不layout绘制了 循环结束
...
//                    mMoreView.layout(dotsXpos, dotsYpos, dotsXpos + dotsViewWidth, dotsYpos + childh);
                    mMoreView.layout(xpos, dotsYpos, xpos + dotsViewWidth, dotsYpos + childh);
                    break;
                }
