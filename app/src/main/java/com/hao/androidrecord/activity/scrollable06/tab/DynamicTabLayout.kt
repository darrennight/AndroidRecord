...
class DynamicTabLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    init {
        ...
        mMode = a.getInt(R.styleable.DynamicTabLayout_dtl_tabMode,
            MODE_SCROLLABLE
        )
        mTabGravity = a.getInt(R.styleable.DynamicTabLayout_dtl_tabGravity,
            GRAVITY_FILL
        )
        a.recycle()
        val res = resources
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (getMode(widthMeasureSpec) != UNSPECIFIED) {
            ...
            tabMaxWidth = if (mRequestedTabMaxWidth > 0) mRequestedTabMaxWidth else specWidth - dpToPx(
    ...
    inner class TabView(context: Context) : LinearLayout(context) {
        ...
        private fun createStoreTabView(storeTabBean: DynamicTabBean): DynamicTabView {
            return DynamicTabView(
                context,
                title = storeTabBean.title,
                descText = storeTabBean.desc,
                tabIcon = storeTabBean.icon
            )
        }
