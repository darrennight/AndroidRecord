...
//		shadowPaint.setMaskFilter(new BlurMaskFilter(10, BlurMaskFilter.Blur.OUTER));
		shadowPaint.setColor(Color.parseColor("#66F1F2F2"));
		shadowPaint.setShadowLayer(1f, 0, 0, Color.parseColor("#F1F2F2"));
		RectF rectF = new RectF();
		rectF.right = getMeasuredWidth();
		rectF.bottom = getMeasuredHeight()- ViewUtils.INSTANCE.dp2px(10);
		canvas.drawRoundRect(rectF,10,10,shadowPaint);

		Path path1 = new Path();
		int arrowHeight = ViewUtils.INSTANCE.dp2px(10);
		path1.moveTo((rectF.right/2)-(arrowHeight/2f), rectF.bottom);
		path1.lineTo((rectF.right / 2), rectF.bottom+(arrowHeight));
		path1.lineTo((rectF.right/2)+(arrowHeight/2f), rectF.bottom);
		path1.close();
