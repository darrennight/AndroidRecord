...
public class BookPageViewTest extends RelativeLayout {
    ...
    public BookPageViewTest(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context,attrs);
    }
...
//        Path path = new Path();
//        path.addRoundRect(new RectF(0, 0, getMeasuredWidth(), getMeasuredHeight()), getMeasuredWidth()/2, getMeasuredHeight()/2, Path.Direction.CW);

//        Path path = getPathAFromTopRight();
//        path.addRect();
...
//        canvas.drawPath(getPathB(),pathBPaint);
    ...
    protected void onDraw(Canvas canvas) {
       ...
       /* if(a.x==-1 && a.y==-1){
            ...
            if(f.x==viewWidth && f.y==0){
                canvas.drawPath(getPathAFromTopRight(),pathAPaint);
            }else if(f.x==viewWidth && f.y==viewHeight){
