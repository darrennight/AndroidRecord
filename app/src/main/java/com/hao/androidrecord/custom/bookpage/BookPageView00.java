package com.hao.androidrecord.custom.bookpage;
 ...
 * 使用版demo1
 */
public class BookPageView00 extends RelativeLayout {
    ...
    public BookPageView00(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context,attrs);
...
//        pathAPaint.setShadowLayer(3f, 0, 3, Color.GRAY);
...
//        pathCPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.DST_ATOP));
        pathCPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_OVER));
//        pathCPaint.setShadowLayer(3f, 0, 3, Color.GRAY);
...
//        pathCPaint.setStyle(Paint.Style.STROKE);
        ...
        testPaintA = new Paint();
        testPaintA.setStyle(Paint.Style.FILL);
        testPaintA.setAntiAlias(true);//设置抗锯齿
        testPaintA.setColor(Color.BLUE);
//        testPaintA.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.DST_OVER));

//        pathCPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
//        pathBPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));

    }

    private Paint testPaintA;
    ...
    private Paint paintClip = new Paint();
    @Override
    protected void dispatchDraw(Canvas canvas) {
       ...
       /* Path mPathXOR = new Path();
        mPathXOR.moveTo(0,0);
        mPathXOR.lineTo(getWidth()-20,0);
        mPathXOR.lineTo(getWidth()-20,getHeight()-20);
        mPathXOR.lineTo(0,getHeight()-20);
        mPathXOR.close();*/
        ...
        if(a.x==-1 && a.y==-1){
//            canvas.drawPaint(pathAPaint);
        }else {
            canvas.drawPath(getPathC(),pathCPaint);
            if(f.x==viewWidth && f.y==0){

//                canvas.clipPath(getPathAFromTopRight(), Region.Op.INTERSECT);
                drawPathAContent(canvas,getPathAFromTopRight());


            }else if(f.x==viewWidth && f.y==viewHeight){
//                canvas.clipPath(getPathAFromLowerRight(), Region.Op.INTERSECT);
...
//            drawPathARightShadow(canvas,pathA);
        ...
        mPathXOR.lineTo(0,getHeight()-20);
        mPathXOR.close();
        canvas.clipPath(mPathXOR, Region.Op.INTERSECT);
